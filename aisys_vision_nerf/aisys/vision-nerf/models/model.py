...
class VisionNerfModel(nn.Module):
    def __init__(self, args, load_opt=True, load_scheduler=True, isactnn=False): # AISYS ADDED
        super(VisionNerfModel, self).__init__()
        self.args = args
        ...
        self.feature_net = VIT(im_feat,
                               ...
                               use_skip_conv=not args.no_skip_conv)
        
        if isactnn: # AISYS ADDED, change the nn.model to actnn.model
            self.net_coarse = actnn.QModule(self.net_coarse)
            if not(args.coarse_only):
                self.net_fine = actnn.QModule(self.net_fine)
            self.feature_net = actnn.QModule(self.feature_net)

        self.net_coarse = self.net_coarse.to(device)
        self.net_fine = self.net_fine.to(device)
    ...
    def encode(self, x):
        ...
        feat_maps = self.feature_net(x.reshape([-1, h, w, c]).permute(0, 3, 1, 2).contiguous())
