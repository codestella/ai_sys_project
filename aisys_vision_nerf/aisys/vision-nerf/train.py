...
def train(args):
    ...
    # Create model
    model = VisionNerfModel(args, load_opt=not args.no_load_opt, load_scheduler=not args.no_load_scheduler, isactnn=args.actnn) # AISYS ADDED, if isactnn, nn.model changed actnn.model
    if args.actnn: # AISYS ADDED
        actnn.set_optimization_level(args.opt_level) # AISYS ADDED, L3: 2-bit
    ...
    # Applying GACT
    #if args.gact:
    #    gact.set_optimization_level(args.opt_level) #L2.2: 2-bit
    ...
    with torch.autograd.graph.saved_tensors_hooks(pack_hook, unpack_hook):
        while global_step < model.start_step + args.n_iters + 1:
            ...
            for train_data in train_loader:
                ...
                # applying gact iteration
                '''if args.gact:
                    def backprop():
                        scalars_to_log_ = {}
                        ray_sampler_ = RaySamplerMultipleImages(train_data, device, global_step, bbox_steps=args.bbox_steps)
                        ray_batch_ = ray_sampler_.random_sample(args.N_rand,
                                                            sample_mode=args.sample_mode,
                                                            center_ratio=args.center_ratio,
                                                            gact = args.gact
                                                            )
                        featmaps_ = model.encode(ray_batch_['src_rgbs']) # (batch, #views, #channels, height', width')

                        ret_ = render_rays(ray_batch=ray_batch_,
                                        model=model,
                                        featmaps=featmaps_,
                                        projector=projector,
                                        N_samples=args.N_samples,
                        ...
                        loss_, scalars_to_log_ = criterion(ret_['outputs_coarse'], ray_batch_, scalars_to_log_)
                        model.optimizer.zero_grad()
                        loss_, scalars_to_log_ = criterion(ret_['outputs_coarse'], ray_batch_, scalars_to_log_)
                        loss_.backward()
                        
                        del loss_
                        del ret_, ray_sampler_, ray_batch_
                        del scalars_to_log_
